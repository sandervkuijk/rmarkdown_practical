...
The markdown language ignores 'enter'. To start a new line, hit space twice (that's what I did just after the link to the book). Plain text could be placed here to introduce the report that you're making. For this tutorial, we will focus on the results section, as this shows best the integration between R code and text. Text can easily be made __bold__, ~~italic~~, or, when you explicitly want to refer to R code but without actually invoking R you can use `mean()`. If you'd like to throw some maths into the mix, you can write simple expressions inline $\sum_{1}^{n}$, or where they are easier to read $$\sum$$. As you can see, this is just an easy formula, but expand as you like!

```{r, echo = FALSE}
d <- read.csv("chronic_pain.csv", sep = ";")
```
...
```{r, echo = FALSE}
perc <- round(sum(d$Sex == 1, na.rm = TRUE)/length(d$ID)*100, 1)
```
For the actual results of analyses, naturally we would start with descriptive statistics. The cohort consisted of `r length(unique(d$ID))` chronic pain patients.  Of all patients, `r sum(d$Sex == 1, na.rm = TRUE)` (`r perc`%) were male, with an average age (standard deviation) of 57.18 (15.14). In case we had many more characteristics, a baseline table would be a better option.
...
Say we were most interested in a simple association between complaints of anxiety and depression in these patients. We could compute the correlation by simply typing `cor.test`, with its associated p-value of p=0.001. However, this is just a measure of the strength of the linear relation between these two variables. this may obscure a more interesting association.
...
fit <- lm(d$Hadssc_a ~ d$Hadssc_d + d$Sex + d$Age)
extract_eq(fit)
```
...
library(stargazer)
stargazer(fit, type = "html", title = "Table 2", single.row = TRUE,
          ci = TRUE, report = "vcsp*", header= FALSE)
```
